...
def process_shapefile(shp_path):
    ...
    for idx, row in gdf.iterrows():
        # Use FIELD_NAME instead of NAME
        polygon_name = row['FIELD_NAME'].replace(".", "_").replace(" ", "_").replace("(", "").replace(")", "")
...
if uploaded_zip is not None:
    ...
    # Check if the extracted folder contains a single extra folder
    temp_dir_path = "temp_dir"
    extracted_items = os.listdir(temp_dir_path)
    if len(extracted_items) == 1 and os.path.isdir(os.path.join(temp_dir_path, extracted_items[0])):
        search_dir = os.path.join(temp_dir_path, extracted_items[0])
    else:
    ...
    for root_dir, _, files in os.walk("temp_dir", topdown=False):
        for file in files:
