...
def process_shapefile(shp_path):
    ...
    # Create the root XML element with static attributes
    ...
    # Adding CTR and FRM elements as static elements
    ctr_element = ET.SubElement(root, "CTR", {"A": "CTR1", "B": "Standard"})
    frm_element = ET.SubElement(root, "FRM", {"A": "FRM1", "B": "Standard", "I": "CTR1"})

    # List to hold all PFD elements
    pfd_elements = []

    for idx, row in gdf.iterrows():
        # Generate an ID for the polygon (instead of using FSNAME)
        polygon_id = f"ID{idx+1}"

        # Create PFD element for each feature with IDs instead of names
        pfd_element = ET.Element("PFD", {
            "A": f"PFD{idx+1}",
            "C": polygon_id,  # Use ID here instead of a modified FSNAME
            "E": "CTR1",
            "F": "FRM1"
        })

        # Create PLN element with the polygon ID
        pln_element = ET.SubElement(pfd_element, "PLN", {
            "A": "1",
            "B": polygon_id  # Use ID here instead of a name
        })

        # Create LSG element
        lsg_element = ET.SubElement(pln_element, "LSG", {"A": "1"})

        # Extract points from the exterior geometry only (ignoring interiors/holes)
    ...
    # Sort the PFD elements by the value of the "C" attribute (which now contains the ID)
...
if uploaded_zip is not None:
    ...
    for root_dir, _, files in os.walk("temp_dir"):
        for file in files:
            if file.endswith(".shp"):
                shp_path = os.path.join(root_dir, file)
                break
    ...
    if shp_path is None:
        ...
        # Process the shapefile and convert it to XML, get EPSG code, and geodata
