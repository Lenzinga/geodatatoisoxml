...
def process_shapefile(shp_path):
    ...
    for idx, row in gdf.iterrows():
        polygon_kg = row['KG'].replace(".", "_").replace(" ", "_").replace("(", "").replace(")", "")
        polygon_gnr = row['GNR'].replace(".", "_").replace(" ", "_").replace("(", "").replace(")", "")
        polygon_name = f"{polygon_kg}_{polygon_gnr}"
...
if uploaded_zip is not None:
    ...
    if shp_path is None:
        ...
        # Process the shapefile and convert it to XML, get EPSG code, KG & GNR values, and geodata
